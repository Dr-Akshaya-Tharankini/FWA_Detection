...
class FWAPredictionEngine:
    ...
    def get_summary_statistics(self, df: pd.DataFrame) -> Dict:
        ...
        # Additional metrics
        incompatible_count = df['incompatible_procedures'].sum()
        duplicate_count = df['duplicate_detected'].sum()
        controlled_substances_count = df['controlled_substances_excessive'].sum()
        suspicious_referral_count = df['suspicious_referral_pattern'].sum()
        frequent_reversals_count = df['frequent_reversals'].sum()
        same_day_visits_total = df['same_day_visits'].sum()
