...
# Header
st.markdown(
    '<p class="main-header" style="font-size:40px; font-weight:700; margin-bottom:5px;">Healthcare Claims FWA Detection System</p>',
    unsafe_allow_html=True
)

st.markdown(
    '<p class="sub-header" style="font-size:24px; font-weight:500; margin-top:-10px;">Fraud, Waste & Abuse Detection for UAE Healthcare Claims</p>',
    unsafe_allow_html=True
)
...
if uploaded_file is not None:
    try:
        ...
        if st.session_state.processed_df is not None:
            ...
            with col1:
                st.metric("Duplicates Detected", summary_stats.get('duplicate_detected_count', 0))
            with col2:
                st.metric("Incompatible Procedures", summary_stats.get('incompatible_procedures_count', 0))
            with col3:
                st.metric("Same-Day Visits", summary_stats.get('same_day_visits_total', 0))
            with col4:
                st.metric("Controlled Substances", summary_stats.get('controlled_substances_excessive_count', 0))
            
            col1, col2, col3, col4 = st.columns(4)
            with col1:
                st.metric("Suspicious Referrals", summary_stats.get('suspicious_referral_pattern_count', 0))
            with col2:
                st.metric("Frequent Reversals", summary_stats.get('frequent_reversals_count', 0))
            with col3:
                st.metric("Providers Flagged", f"{summary_stats.get('providers_flagged', 0)}/{summary_stats.get('total_providers', 0)}")
            with col4:
                providers_flagged = summary_stats.get('providers_flagged', 0)
                total_providers = summary_stats.get('total_providers', 0)
                flagged_pct = (providers_flagged / total_providers * 100) if total_providers > 0 else 0
